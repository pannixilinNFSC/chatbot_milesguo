...
```
chatbot_milesguo/
  server.py                 # Simple server entry (web demo)

  chatbot.ipynb             # Main chatbot (RAG + LLM) notebook
  search.ipynb              # Search-only notebook
  data_prepare.ipynb        # Data preparation pipeline notebook

  lib/                      # Core Python utilities used by notebooks/server
    __init__.py
    chatbot_utils.py        # Chatbot/RAG helper functions
    openai_utils.py         # OpenAI API helpers
    search_utils.py         # Vector search helpers (FAISS, etc.)
    search_utils_word.py    # Word-based search helpers

  prompt.json               # Prompt templates / system instructions
  titles.json               # Title metadata
  dict_emb.json             # Cached embeddings / embedding dictionary
